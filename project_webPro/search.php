...
if ($select == "3" || $select == "2") {
    // สถานที่ท่องเที่ยว
    $sql = "
    SELECT p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', link FROM picofplace as picp INNER JOIN place as p on picp.id_pln = p.id INNER JOIN village as v on p.id_vill = v.id INNER JOIN sub_district as sd on v.id_dis = sd.id WHERE (p.id_ctgry = 2 OR p.id_ctgry = 3) AND ( p.name LIKE '%" . $str . "%' OR v.name LIKE '%" . $str . "%' OR sd.name LIKE '%" . $str . "%' ) GROUP BY p.id;
    ";
} else if ($select == "5") {
    // ที่พัก
    $sql = "
    SELECT p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', link FROM picofplace as picp INNER JOIN place as p on picp.id_pln = p.id INNER JOIN village as v on p.id_vill = v.id INNER JOIN sub_district as sd on v.id_dis = sd.id WHERE (p.id_ctgry = 5) AND ( p.name LIKE '%" . $str . "%' OR v.name LIKE '%" . $str . "%' OR sd.name LIKE '%" . $str . "%' ) GROUP BY p.id;
    ";
} else if ($select == "1" || $select == "4") {
    // ร้านค้าและร้านอาหาร
    $sql = "
    SELECT p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', link FROM picofplace as picp INNER JOIN place as p on picp.id_pln = p.id INNER JOIN village as v on p.id_vill = v.id INNER JOIN sub_district as sd on v.id_dis = sd.id WHERE (p.id_ctgry = 1 OR p.id_ctgry = 4) AND ( p.name LIKE '%" . $str . "%' OR v.name LIKE '%" . $str . "%' OR sd.name LIKE '%" . $str . "%' ) GROUP BY p.id;
    ";
} else if ($select == "0") {
    // สินค้า
    $sql = "
    SELECT pro.id as 'pro_id', p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', pro.name as 'name_product', pro.detail as 'detail', link
    FROM picofproducts as picp 
    INNER JOIN products as pro ON picp.id = pro.id
    INNER JOIN place as p ON pro.id_pln = p.id
    INNER JOIN village as v ON p.id = v.id
    INNER JOIN sub_district as sd ON v.id_dis = sd.id
    WHERE (p.id_ctgry = 1 OR p.id_ctgry = 4) AND ( p.name LIKE '%" . $str . "%' OR v.name LIKE '%" . $str . "%' OR sd.name LIKE '%" . $str . "%' OR pro.name LIKE '%" . $str . "%') GROUP BY pro.id;
    ";
}
                ...
                ?>
                        <div class="col-sm-4 mb-4">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <h5 class="card-title"> <?php echo $objResult["name_product"] ?> </h5>
                                    <p>
                                        <?php echo $objResult["name_place"]; ?> <br>
                                    ...
                                    <a href="product_detail_page.php?pro_id=<?php echo $objResult['pro_id'] ?>,pro_name=<?php echo $objResult['name_product'] ?>" class="btn btn-primary">ดูเพิ่มเติม</a>
                                </div>
