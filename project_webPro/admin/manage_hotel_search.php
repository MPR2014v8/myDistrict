...
$str = $_POST["search_hotel"];

$sql = "SELECT p.id as 'id', p.name as 'name', lat, lng, v.name as 'village', c.name as 'ctgry', detail FROM `place` as p INNER JOIN village as v ON p.id_vill = v.id INNER JOIN categories as c ON p.id_ctgry = c.id WHERE id_ctgry = 5 AND( p.name LIKE '%" . $str . "%' OR v.name LIKE '%" . $str . "%' OR c.name LIKE '%" . $str . "%' OR lat LIKE '%" . $str . "%' OR lng LIKE '%" . $str . "%');";
