...
<head>
    ...
    <title>บันทึกข้อมูลที่พัก</title>
...
<body>
    ...
    <div class="container my-3">
        ...
        <h2 class="text-center">แบบฟอร์มบันทึกข้อมูลที่พัก</h2>
        <form action="../manage/insertData_place.php" method="post">
            ...
            <label for="ctgry">ประเภทสถานที่</label>
                <?php
                $sql = "SELECT name FROM categories WHERE id = 5";
                $result = mysqli_query($conn, $sql);
                ?>
                <select name="ctgry" class="form-control" required>
                    <option value="ที่พัก">เลือกประเภท</option>
                    <?php foreach ($result as $results) { ?>
                        <option value="<?php echo $results["name"]; ?>">
                            <?php echo $results["name"]; ?>
                        </option>
            ...
            <button class="btn btn-success my-2" name="insert_hotel" type="submit">เพิ่มข้อมูลที่พัก</button>
