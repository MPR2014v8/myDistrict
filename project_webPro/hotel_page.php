...
$sql = "
SELECT p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', link FROM picofplace as picp INNER JOIN place as p on picp.id_pln = p.id INNER JOIN village as v on p.id_vill = v.id INNER JOIN sub_district as sd on v.id_dis = sd.id WHERE p.id_ctgry = 5 GROUP BY p.id;
";
