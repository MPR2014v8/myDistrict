...
<body>
  ...
  <section id="festival" class="py-5">
    ...
    $sql = "
    SELECT pro.id as 'pro_id', p.id as 'p_id', p.name as 'name_place', lat, lng, v.name as 'name_village', sd.name as 'name_sub_dis', pro.name as 'name_product', pro.detail as 'detail', link FROM picofproducts as picp INNER JOIN products as pro ON picp.id = pro.id INNER JOIN place as p ON pro.id_pln = p.id INNER JOIN village as v ON p.id = v.id INNER JOIN sub_district as sd ON v.id_dis = sd.id WHERE (p.id_ctgry = 1 OR p.id_ctgry = 4) GROUP BY pro.id;
      ";
